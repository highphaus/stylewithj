...
export default function ServicesContent({ 
  ...
  return (
    <div className={isEmbedded ? "w-full" : "min-h-screen bg-[#FAF9F6] text-[#1A1A1A]"}>
      {!isEmbedded && <Navigation />}

      {/* ── TOP HERO HEADER (MINIMALIST TYPOGRAPHY) ── */}
      <section className={`${isEmbedded ? "pt-16 sm:pt-24" : "pt-36 sm:pt-44"} pb-16 px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto border-b border-black/10`}>
      ...
      {!isEmbedded && <AtelierFooter hideCta={hideCta} />}
    </div>
  );
}
