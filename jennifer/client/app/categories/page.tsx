...
export default function CategoriesPage() {
  return (
    <div className="min-h-screen bg-[#FAF9F6] text-[#1A1A1A]">
      ...
      {/* ── EDITORIAL HEADER ── */}
      <header className="pt-36 sm:pt-44 pb-14 sm:pb-20 px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto border-b border-black/10">
      ...
      <main className="px-5 sm:px-12 lg:px-20 max-w-7xl mx-auto py-12 sm:py-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
          {categoryData.map((cat) => (
            <div 
              key={cat.num}
              className="bg-[#FAF8F3] border border-black/10 p-6 sm:p-10 shadow-[0_10px_35px_rgba(0,0,0,0.02)] rounded-xs flex flex-col justify-between group hover:border-black/30 transition-all duration-300"
            >
              <div>
                {/* Header Badge */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-6 items-start">
                  ...
                  {/* Left Column: Image */}
                  <div className="sm:col-span-5 relative aspect-[3/4] w-full overflow-hidden bg-[#EFECE6] border border-black/5 rounded-xs shadow-xs">
                    <Image
                      src={cat.image}
                      alt={cat.title}
                      fill
                  ...
                  <div className="sm:col-span-7 flex flex-col gap-4">
                    <h3 className="font-serif text-xl sm:text-2xl font-light text-black leading-snug">
                      {cat.headline}
                    </h3>
                    ...
                    <div className="pt-2 border-t border-black/10">
                      <span className="text-[9px] tracking-[0.25em] uppercase text-black/45 block mb-3 font-semibold">
                        Included Services:
                      </span>
                      <div className="flex flex-wrap gap-2">
                        {cat.items.map((item, idx) => (
                          <span 
                            key={idx}
                            className="inline-flex items-center gap-1.5 text-[11px] font-sans font-light bg-white border border-black/10 px-3 py-1.5 rounded-xs text-black/80 shadow-2xs"
                          >
              ...
              <div className="mt-8 pt-4 border-t border-black/10 flex justify-between items-center">
                <Link 
                  href="/services" 
                  className="font-sans text-[10px] tracking-[0.25em] uppercase font-medium text-black hover:opacity-70 transition-opacity flex items-center gap-2"
                >
                  Explore Details
                ...
                <Link 
                  href="/connect" 
                  className="px-4 py-2 bg-black text-white text-[9px] tracking-[0.25em] uppercase font-light hover:bg-black/80 transition-colors rounded-xs"
                >
                  Book Session
                </Link>
              </div>
        ...
        <section className="mt-16 sm:mt-24 p-8 sm:p-14 bg-[#EFECE6] border border-black/10 rounded-xs flex flex-col sm:flex-row justify-between items-start sm:items-center gap-8 shadow-sm">
          <div className="max-w-2xl">
            <span className="text-[9px] tracking-[0.4em] uppercase font-sans text-black/45 font-semibold block mb-2">
              PERSONALIZED CONSULTATIONS
            </span>
            <h2 className="font-serif text-2xl sm:text-4xl font-light text-black leading-tight">
              Ready to find a style that feels like you, only better?
            </h2>
          </div>
          <Link
            href="/connect"
            className="group flex items-center gap-6 px-8 py-4 bg-black text-white text-[10px] tracking-[0.3em] uppercase font-light hover:bg-black/85 transition-all shadow-md flex-shrink-0 rounded-xs"
          >
